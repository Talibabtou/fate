...
This document describes the mainnet gate. The deterministic fixture is for localnet and devnet custody testing only.
...
Fate can make the variable authority a program PDA and expose permissionless wrapper instructions. A connected user or independent public caller can then advance the variable without becoming a trusted randomness operator.
...
The exact target-slot offset must come from the reviewed mainnet implementation and controlled or forked production-path tests. It should leave enough time for ordinary caller and provider delay while remaining well inside the recent slot-hash window. No Entropy devnet deployment is required.
