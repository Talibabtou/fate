...
3. `Locked` on localnet/devnet: derive the deterministic entropy result, locate the selected per-wallet positions, supply both verified weight paths, and pay rent for the next `Draw`.
