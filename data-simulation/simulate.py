...
# These lamport/BPS helpers mirror the protocol's integer math. The scenario
# model below intentionally remains float-based, but these functions provide a
# byte-for-byte fixture for the values that cross the program boundary.
PROTOCOL_BPS_DENOMINATOR = 10_000
PROTOCOL_INITIAL_THRESHOLD_BPS = 100
PROTOCOL_RELATIVE_ACTIVATION_FLOOR_BPS = 10
PROTOCOL_THRESHOLD_DECAY_BPS = 9_000
PROTOCOL_THRESHOLD_DECAY_INTERVAL_SECONDS = 600
PROTOCOL_MINIMUM_DRAW_POOL_LAMPORTS = 100_000_000
PROTOCOL_STAKER_EROSION_BPS = 7
PROTOCOL_PLAYER_EROSION_CAP_BPS = 700
PROTOCOL_PROTOCOL_FEE_BPS = 500
PROTOCOL_BASE_PLAYER_BOOST_BPS = 10_000
PROTOCOL_MAX_EARLY_BOOST_BPS = 5_000
PROTOCOL_MAX_PLAYER_BOOST_BPS = 15_000
PROTOCOL_STAKER_JACKPOT_BPS = 3_000
PROTOCOL_STAKER_PRO_RATA_BPS = 6_500


def _protocol_mul_div_floor(value: int, numerator: int, denominator: int) -> int:
    if denominator == 0:
        raise ValueError("protocol division by zero")
    return value * numerator // denominator


def protocol_activation_threshold(staker_tvl_lamports: int, elapsed_funding_seconds: int) -> int:
    floor = max(
        _protocol_mul_div_floor(
            staker_tvl_lamports,
            PROTOCOL_RELATIVE_ACTIVATION_FLOOR_BPS,
            PROTOCOL_BPS_DENOMINATOR,
        ),
        PROTOCOL_MINIMUM_DRAW_POOL_LAMPORTS,
    )
    threshold = max(
        _protocol_mul_div_floor(
            staker_tvl_lamports,
            PROTOCOL_INITIAL_THRESHOLD_BPS,
            PROTOCOL_BPS_DENOMINATOR,
        ),
        floor,
    )
    for _ in range(elapsed_funding_seconds // PROTOCOL_THRESHOLD_DECAY_INTERVAL_SECONDS):
        if threshold <= floor:
            return floor
        threshold = max(
            _protocol_mul_div_floor(threshold, PROTOCOL_THRESHOLD_DECAY_BPS, PROTOCOL_BPS_DENOMINATOR),
            floor,
        )
    return max(threshold, floor)


def protocol_player_boost_bps(
    player_tvl_before_deposit: int,
    initial_threshold_lamports: int,
    is_activated: bool,
) -> int:
    if is_activated or initial_threshold_lamports == 0:
        return PROTOCOL_BASE_PLAYER_BOOST_BPS
    remaining = max(initial_threshold_lamports - player_tvl_before_deposit, 0)
    early_boost = _protocol_mul_div_floor(
        remaining,
        PROTOCOL_MAX_EARLY_BOOST_BPS,
        initial_threshold_lamports,
    )
    return min(PROTOCOL_BASE_PLAYER_BOOST_BPS + early_boost, PROTOCOL_MAX_PLAYER_BOOST_BPS)


def protocol_boosted_player_weight(deposit_lamports: int, boost_bps: int) -> int:
    return _protocol_mul_div_floor(deposit_lamports, boost_bps, PROTOCOL_BPS_DENOMINATOR)


def protocol_staker_erosion(staker_tvl_lamports: int, player_tvl_lamports: int) -> int:
    return min(
        _protocol_mul_div_floor(staker_tvl_lamports, PROTOCOL_STAKER_EROSION_BPS, PROTOCOL_BPS_DENOMINATOR),
        _protocol_mul_div_floor(player_tvl_lamports, PROTOCOL_PLAYER_EROSION_CAP_BPS, PROTOCOL_BPS_DENOMINATOR),
    )


def protocol_player_settlement(
    staker_tvl_lamports: int,
    player_tvl_lamports: int,
    winner_deposit_lamports: int,
) -> tuple[int, int, int, int, int, int]:
    if winner_deposit_lamports > player_tvl_lamports:
        raise ValueError("winner deposit exceeds Player TVL")
    losing_player = player_tvl_lamports - winner_deposit_lamports
    erosion = protocol_staker_erosion(staker_tvl_lamports, player_tvl_lamports)
    gross_profit = losing_player + erosion
    winner_profit = _protocol_mul_div_floor(
        gross_profit,
        PROTOCOL_BPS_DENOMINATOR - PROTOCOL_PROTOCOL_FEE_BPS,
        PROTOCOL_BPS_DENOMINATOR,
    )
    return (
        losing_player,
        ...
        gross_profit,
        winner_profit,
        winner_deposit_lamports + winner_profit,
        gross_profit - winner_profit,
    )


def protocol_staker_settlement(player_tvl_lamports: int) -> tuple[int, int, int]:
    jackpot = _protocol_mul_div_floor(
        player_tvl_lamports, PROTOCOL_STAKER_JACKPOT_BPS, PROTOCOL_BPS_DENOMINATOR
    )
    pro_rata = _protocol_mul_div_floor(
        player_tvl_lamports, PROTOCOL_STAKER_PRO_RATA_BPS, PROTOCOL_BPS_DENOMINATOR
    )
    return jackpot, pro_rata, player_tvl_lamports - jackpot - pro_rata
